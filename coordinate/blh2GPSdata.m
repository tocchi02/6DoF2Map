function [] = blh2GPSdata(fileaddress,time,blh,time_ref,day_ref)
	...
	for i = 1:len
		...
		latitude = blh_deg2blh_GPSformat(blh(i,1));		% 緯度
		longitude = blh_deg2blh_GPSformat(blh(i,2));	% 経度
		height = blh(i,3);	% 高度
		str1 = '$GPGGA,';
		str2 = sprintf('%09.2f,%10.6f,N,%011.6f,', time_UTC, latitude, longitude);
		str3 = sprintf('E,1,08,0.9,%.2f,M,28.4,M,,,*', height);	%作りこみが甘い
...
function output = elapsedtime2GPS_UTC(time,time_ref)
	% 基準時刻(time_ref)とコンピュータ起動時間(time)を足してタイムスタンプ生成（UTC）
